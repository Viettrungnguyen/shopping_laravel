...
export default {
  ...
  methods: {
    ...
    async changePassword() {
      await this.$axios
        .post('user/profile/change-password/' + this.user.id, {
    ...
    async updateProfile() {
      ...
      formData.append('avatar_url', this.selectFile, this.selectFile.name)
      ...
      await this.$axios
        .post('user/profile/update/' + this.user.id, formData)
        .then((res) => {
          // eslint-disable-next-line no-console
          console.log(res)
          if (res.status === 200) {
